...
- **Adaptación a celulares y tablets:** En celulares, los controles se adaptan verticalmente con una barra de navegación inferior cómoda para usar con el pulgar. Esa barra reúne **En Vivo**, **Escena**, **Combate**, **Momentos** y **Más**; dentro de **Más** se abre una hoja inferior deslizable con **Herramientas de mesa**, **Notas y dados** y **Campaña**. En tablets horizontales, la pantalla se organiza en dos columnas para mantener el escenario 16:9 visible a la izquierda mientras desplazás los controles a la derecha. La interfaz respeta los recortes de cámara frontal (*notch*) y las barras del sistema operativo (*áreas seguras*).
- **Pantalla activa sin bloqueos (Wakelock):** Mientras estés dirigiendo en el Control Remoto, la app mantiene la pantalla encendida de forma continua para que el teléfono no se apague ni se bloquee por inactividad a mitad de una escena o combate.
- **Botón Atrás de Android:** En dispositivos móviles, el botón o gesto Atrás nativo no cierra la app de golpe. Sigue un orden seguro: primero cierra el teclado virtual si estás escribiendo, luego cancela arrastres o cierra paneles y ventanas emergentes (incluyendo los formularios de combate y previsualizaciones), y finalmente pide confirmación antes de salir al menú inicial tanto en el Control como en la Pantalla de Escena.
- **Compartir el acceso de la sala:** En **Mostrar Código QR y PIN**, tocá **Compartir enlace** para enviar el acceso mediante la hoja nativa de Android. En navegador, si no hay hoja de compartir disponible, el enlace se copia automáticamente.
- **Cambios rápidos durante la partida:** En la pestaña **En Vivo**, usá **Acciones rápidas** para activar con un toque **Relámpago**, **Sacudir**, **Cartel**, **Ambiente** y **Sonidos**. Tocá **Más** y luego **Herramientas de mesa** para abrir acciones secundarias como **Iluminación**, **Cámara y escena**, **Mostrar recurso** y **Música ambiental**. La hoja inferior se puede arrastrar hacia abajo, tocar afuera para cerrarla o elegir una acción.
...
- **Herramientas de mesa en el celular:** En la barra inferior tocá **Más** y luego **Herramientas de mesa**. Desde esa hoja inferior podés moverte por las pestañas **Publicar**, **Escena**, **Partida** y **Sistema**. Ahí podés cambiar entre **En vivo** y **Preparación**, publicar o descartar cambios, abrir escena y cámara, acceder a recursos, combate, historial, presets, diagnóstico y **Modo Partida**.
...
- **Panel de Control Modular e Inspector:** Segunda opción del selector de sesión, recomendada cuando necesitás interactuar directamente sobre las figuras del escenario:
  1. **Escenario 16:9 persistente:** Ubicado en la parte superior sin recortes ni deformaciones, muestra fielmente lo que ve la Mesa. Podés tocar directamente cualquier figura para seleccionarla o arrastrarla suavemente para reubicarla en directo con **snapping magnético inteligente** (se imanta a las casillas si la cuadrícula está encendida, o a la línea de suelo de los compañeros si está apagada). Arriba a la derecha contás con el conmutador de **Cuadrícula táctica**, el botón de **Formaciones rápidas** (icono de destello) y el botón para **Maximizar / Pantalla completa**.
  ...
  3. **Inspector contextual de personaje:** Al tocar una figura, los módulos se reemplazan por su ficha de edición rápida: muestra su avatar con anillo luminoso, nombre, interruptor **Visible en mesa**, controles de **Tamaño** (`-`/`+`), presets rápidos (**Mini** 0.25x para encajar en casillas, **Peq** 0.6x, **Med** 1.0x, **Gra** 1.4x, **Enor** 1.9x) y deslizador continuo ampliado desde **15%** hasta **250%** (permitiendo miniaturas reales sin topes artificiales), selector de **Formato visual** (**Auto**, **Standee**, **Token** para forzar individualmente si se proyecta como figura completa o ficha circular), **Capa / Orden** (`▲`/`▼`), botón **Hablar…** para abrir de inmediato la ventana de globos de diálogo (JRPG / cinemáticos), botón **Editar Ficha**, botón **Espejo** para reflejar la figura y botón **Volver al panel modular** para regresar. El escalado se calcula en unidades proporcionales a la altura del escenario, de modo que una figura al 100% ocupa la misma proporción visual tanto en un teléfono pequeño como en una pantalla 4K.
  4. **Lienzo táctico móvil, Tokens VTT y Formaciones Rápidas:**
     - **Menú de Formaciones Rápidas:** Al tocar el botón de destellos junto a la cuadrícula, se despliegan acciones táctiles inmediatas: **Fila de Batalla (JRPG)** (alinea héroes a la izquierda mirando a la derecha y enemigos a la derecha mirando a la izquierda con tamaño proporcionado a la cantidad de combatientes), **Alinear a Cuadrícula** (centra todas las figuras en sus casillas más cercanas), **Distribuir en Línea** (reparte a los personajes horizontalmente con separación uniforme) y **Ajustar Tamaño a Casilla** (escala automáticamente las figuras para encajar de forma óptima en 1 celda del mapa).
     - **Modo Tokens VTT:** Al activar la **Cuadrícula táctica**, tanto en tu control como en la pantalla de la Mesa los personajes se convierten en fichas circulares tácticas (tokens) con código de bando (verde para aliados, rojo para enemigos, amarillo para neutrales), nombre compacto inferior y barra de vida de un vistazo si están en combate. Al arrastrar un token, una línea punteada y un badge flotante indican la distancia en celdas y pies (`1 celda = 5 ft`); al soltarlo, se imanta al centro de la casilla.
...
2. En **Control Remoto**, escribí el código de la Mesa (las letras se convierten automáticamente a mayúsculas) y tocá la flecha **Conectar** o presioná **Enter** en el teclado virtual.
...
**Resultado esperado:** el director tiene sus controles y la Mesa queda lista para recibir los cambios. Comprobamos la conexión por código y WebRTC entre dispositivos Android físicos (celular y tablet); el escaneo de cámara y la aprobación manual cuando se solicite conservan sus pasos respaldados.
